...
const copy: Record<string, [string, string, string]> = {
  ...
  timeline: ['07 / Timeline', '36 HOURS. ONE CONTINUOUS BUILD.', 'Event sequence only. Official timing details will be added once confirmed.'],
...
function PageContent({ page }: Props) {
  ...
  if (page === 'prizes') return <section className="page-content prize-page"><div className="pool"><p>Total prize pool</p><b>₹5L+</b><span>Confirmed headline amount</span></div><div className="prize-list light">{[['01', 'Winner', '₹65,000'], ['02', 'Runner up', '₹50,000'], ['03', '2nd runner up', '₹30,000']].map(([rank, title, amount]) => <article key={rank}><small>{rank}</small><span>◉</span><b>{title}</b><strong>{amount}</strong></article>)}</div><p className="temporary">Additional prizes, benefits and awards: TBA pending organizer confirmation.</p></section>
